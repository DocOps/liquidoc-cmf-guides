...
[NOTE]
Git is the fully open-source _distributed source control system_ (DVCS), whereas GitHub is a cloud-hosted commercial application built around Git, similar to other commercial projects like GitLab and BitBucket.
These products add value to Git with web interfaces, collaborative features, and peripheral tooling.
Most of the instructions here relate to Git, not GitHub, and therefore can be applied regardless of which platform you use.
...
[TIP.beginner]
.A note on Windows & Ruby
Windows is not a great environment for coding, but you can definitely make it work.
We recommend using the link:https://gitforwindows.org/[GitBash terminal] for both Git and Ruby commands, the latter usage requiring an link:https://stackoverflow.com/questions/3426347/how-to-run-ruby-and-git-commands-in-one-place-on-windows/10159798#10159798[additional setup step].
...
Browse link:http://127.0.0.1:{local_serve_port}/index[http://127.0.0.1:{local_serve_port}/index].
// end::admin-edit[]
